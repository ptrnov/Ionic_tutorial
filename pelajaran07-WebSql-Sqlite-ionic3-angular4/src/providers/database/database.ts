...
export class DatabaseProvider {
        ...
        if (platform._platforms[0] == 'cordova') {
          ...
          // /if (this.isOpen){
            ...
            this.storage.create({ name: DB_NAME ,location:"default" }).then( ( db: SQLiteObject ) => {
                ...
                // db.executeSql("INSERT INTO piter (id,usr,psw)  VALUES (NULL,'admin1','12345')",{}).then(()=>{
  ...
  /* Punsi Tambah Data untuk WebSql Browser
  ...
  */
  ...
  selectData(){  
    return new Promise(resolve => {    
      this.platform.ready().then(() => {
        console.log('platform Select Indentification');      
        if (this.platform._platforms[0] == 'cordova') {
         
            console.log('Sqlite cordova/Mobile Flatform');
            let qrySelect ="SELECT id, usr FROM piter ORDER BY id DESC";
